...
void esp_h264_enc_hw_rc_set_qp(esp_h264_rc_hd_t rc_hd, uint8_t qp_max, uint8_t qp_min)
{
    ...
    prc->qp_max = qp_max;
    prc->qp_min = qp_min;
    prc->qpm = CLIP3(prc->qp_min, prc->qp_max, prc->qpm);
    prc->qp_average_frame = CLIP3(prc->qp_min, prc->qp_max, prc->qp_average_frame);
}
...
void esp_h264_rc_start(esp_h264_rc_hd_t rc_hd, bool is_iframe, uint32_t *rate, uint32_t *pred_mad, uint8_t *qp)
{
    ...
    int32_t target_frame_bits = ((int32_t)prc->bits_per_frame * 10 -
                                 (int32_t)prc->frame_bits_last4_average * 4) / 6;
    int32_t min_target_frame_bits = (int32_t)prc->bits_per_frame / 4;
    int32_t max_target_frame_bits = (int32_t)prc->bits_per_frame * 2;
    int target_mb_bits = 0;

    if (min_target_frame_bits < 1) {
        min_target_frame_bits = 1;
    }
    if (max_target_frame_bits < min_target_frame_bits) {
        max_target_frame_bits = min_target_frame_bits;
    }
    target_frame_bits = CLIP3(min_target_frame_bits, max_target_frame_bits, target_frame_bits);
    ...
    uint8_t qp_average_frame = prc->qp_average_frame != 0 ? prc->qp_average_frame : prc->qpm;
    prc->qpm = (uint32_t)qp_average_frame + (int)prc->eqp;
