...
本文对应 Device Monitor `1.0.0`。先完成板型和环境核对，再构建、联网和验证业务。
本仓提供源码，以下烧录步骤仅供开发者使用自己的构建产物。
...
- 应用 BIN 不能单独写到 `0x0`；本项目没有提供从 `0x0` 烧录的完整镜像。
...
| OTA | 按部署环境核对请求、版本、下载失败及成功后的启动状态 |
| 网络与长稳 | 正常网与双向弱网分别记录丢包、时延、帧率、队列和内存水位 |
| 摄像头负载 | 静态画面与动态高运动画面分别测试，不用静态低码率推断性能余量 |

媒体目标见 [默认媒体参数](../README.md#默认媒体参数)。
音频底板未适配，音频、AI 对讲和 AEC 不列入已支持功能。
...
局域网抓屏服务默认关闭。呼叫、挂断、Wi-Fi 切换、OTA 等会改变状态的命令，
