...
static esp_err_t req_get_coprocessor_fw_version(Rpc *req, Rpc *resp, void *priv_data)
{
	...
	uint32_t promisc_unique = 0;
	uint32_t promisc_retries = 0;
	...
	c61_udp_diag_take_snapshot(&udp_packets, &promisc_unique, &promisc_retries);
	resp_payload->revision   = (int32_t)udp_packets;
	resp_payload->prerelease = (int32_t)promisc_unique;
	resp_payload->build      = (int32_t)promisc_retries;
