...
#define C61_UDP_DIAG_PORT 5005U
#define C61_UDP_DIAG_MAGIC 0x43363144U
#define C61_UDP_DIAG_MACSTR "%02x:%02x:%02x:%02x:%02x:%02x"
#define C61_UDP_DIAG_MAC2STR(a) (a)[0], (a)[1], (a)[2], (a)[3], (a)[4], (a)[5]

static portMUX_TYPE s_c61_udp_diag_lock = portMUX_INITIALIZER_UNLOCKED;
static uint32_t s_c61_udp_diag_packets;
static uint32_t s_c61_udp_diag_max_forward_us;
static uint32_t s_c61_udp_diag_promisc_unique;
static uint32_t s_c61_udp_diag_promisc_retries;
static uint16_t s_c61_udp_diag_promisc_last_seq[16];
static uint16_t s_c61_udp_diag_promisc_seen_mask;
static uint8_t s_c61_udp_diag_sta_mac[6];

static uint16_t c61_udp_diag_read_be16(const uint8_t *data)
{
	return ((uint16_t)data[0] << 8) | data[1];
}

static uint32_t c61_udp_diag_read_be32(const uint8_t *data)
{
	return ((uint32_t)data[0] << 24) | ((uint32_t)data[1] << 16) |
		((uint32_t)data[2] << 8) | data[3];
}

static bool c61_udp_diag_observe(const void *buffer, uint16_t len)
{
	const uint8_t *packet = buffer;
	if (len < 14U + 20U + 8U + 12U || c61_udp_diag_read_be16(packet + 12) != 0x0800U) {
		return false;
	}

	const uint8_t ihl = (packet[14] & 0x0fU) * 4U;
	if (ihl < 20U || len < 14U + ihl + 8U + 12U || packet[23] != 17U) {
		return false;
	}

	const uint8_t *udp = packet + 14U + ihl;
	const uint8_t *payload = udp + 8U;
	if (c61_udp_diag_read_be16(udp + 2) != C61_UDP_DIAG_PORT ||
		c61_udp_diag_read_be32(payload) != C61_UDP_DIAG_MAGIC) {
		return false;
	}

	portENTER_CRITICAL(&s_c61_udp_diag_lock);
	s_c61_udp_diag_packets++;
	portEXIT_CRITICAL(&s_c61_udp_diag_lock);
	return true;
}

static uint16_t c61_udp_diag_read_le16(const uint8_t *data)
{
	return ((uint16_t)data[1] << 8) | data[0];
}

static void c61_udp_diag_promisc_cb(void *buffer, wifi_promiscuous_pkt_type_t type)
{
	if (!buffer || type != WIFI_PKT_DATA) {
		return;
	}

	const wifi_promiscuous_pkt_t *packet = buffer;
	const uint8_t *frame = packet->payload;
	if (packet->rx_ctrl.rx_state != 0 || packet->rx_ctrl.sig_len < 24U) {
		return;
	}

	const uint16_t frame_control = c61_udp_diag_read_le16(frame);
	const uint8_t frame_type = (frame_control >> 2) & 0x03U;
	const uint8_t subtype = (frame_control >> 4) & 0x0fU;
	const bool to_ds = (frame_control & (1U << 8)) != 0;
	const bool from_ds = (frame_control & (1U << 9)) != 0;
	if (frame_type != 2U || to_ds || !from_ds || (subtype & 0x04U) != 0 ||
		memcmp(frame + 4, s_c61_udp_diag_sta_mac, sizeof(s_c61_udp_diag_sta_mac)) != 0) {
		return;
	}

	uint8_t tid = 0;
	if ((subtype & 0x08U) != 0 && packet->rx_ctrl.sig_len >= 26U) {
		tid = frame[24] & 0x0fU;
	}
	const uint16_t sequence_control = c61_udp_diag_read_le16(frame + 22);
	const uint16_t tid_bit = (uint16_t)(1U << tid);

	portENTER_CRITICAL(&s_c61_udp_diag_lock);
	if ((frame_control & (1U << 11)) != 0) {
		s_c61_udp_diag_promisc_retries++;
	}
	if ((s_c61_udp_diag_promisc_seen_mask & tid_bit) == 0 ||
		s_c61_udp_diag_promisc_last_seq[tid] != sequence_control) {
		s_c61_udp_diag_promisc_last_seq[tid] = sequence_control;
		s_c61_udp_diag_promisc_seen_mask |= tid_bit;
		s_c61_udp_diag_promisc_unique++;
	}
	portEXIT_CRITICAL(&s_c61_udp_diag_lock);
}
...
void c61_udp_diag_promisc_start(void)
{
	...
	esp_err_t ret = esp_wifi_get_mac(WIFI_IF_STA, s_c61_udp_diag_sta_mac);
	if (ret == ESP_OK) {
		ret = esp_wifi_set_promiscuous_filter(&filter);
	}
	if (ret == ESP_OK) {
		ret = esp_wifi_set_promiscuous_rx_cb(c61_udp_diag_promisc_cb);
	}
	if (ret == ESP_OK) {
		ret = esp_wifi_set_promiscuous(true);
	}
	ESP_LOGW(TAG, "C61 UDP pre-netif diagnostic: enable=%s sta=" C61_UDP_DIAG_MACSTR,
		esp_err_to_name(ret), C61_UDP_DIAG_MAC2STR(s_c61_udp_diag_sta_mac));
}

void c61_udp_diag_promisc_stop(void)
{
	const esp_err_t ret = esp_wifi_set_promiscuous(false);
	ESP_LOGW(TAG, "C61 UDP pre-netif diagnostic: disable=%s", esp_err_to_name(ret));
}

static void c61_udp_diag_record_forward_time(int64_t started_us)
{
	const uint32_t elapsed_us = (uint32_t)(esp_timer_get_time() - started_us);
	portENTER_CRITICAL(&s_c61_udp_diag_lock);
	if (elapsed_us > s_c61_udp_diag_max_forward_us) {
		s_c61_udp_diag_max_forward_us = elapsed_us;
	}
	portEXIT_CRITICAL(&s_c61_udp_diag_lock);
}

void c61_udp_diag_take_snapshot(uint32_t *packets,
		uint32_t *promisc_unique, uint32_t *promisc_retries)
{
	portENTER_CRITICAL(&s_c61_udp_diag_lock);
	*packets = s_c61_udp_diag_packets;
	*promisc_unique = s_c61_udp_diag_promisc_unique;
	*promisc_retries = s_c61_udp_diag_promisc_retries;
	s_c61_udp_diag_packets = 0;
	s_c61_udp_diag_promisc_unique = 0;
	s_c61_udp_diag_promisc_retries = 0;
	s_c61_udp_diag_max_forward_us = 0;
	memset(s_c61_udp_diag_promisc_last_seq, 0,
		sizeof(s_c61_udp_diag_promisc_last_seq));
	s_c61_udp_diag_promisc_seen_mask = 0;
	portEXIT_CRITICAL(&s_c61_udp_diag_lock);
}
...
#define C61_WIFI_TX_BACKPRESSURE_US       (50 * 1000)
...
{
	interface_buffer_handle_t buf_handle = {0};
	const bool udp_diag_packet = buffer && c61_udp_diag_observe(buffer, len);
	const int64_t udp_diag_started_us = udp_diag_packet ? esp_timer_get_time() : 0;
	if (c61_rx_probe_observe(buffer, len)) {
		if (eb) {
			esp_wifi_internal_free_rx_buffer(eb);
		}
		return ESP_OK;
	...
	if (unlikely(send_to_host_queue(&buf_handle, PRIO_Q_OTHERS)))
		goto DONE;
	if (udp_diag_packet) {
		c61_udp_diag_record_forward_time(udp_diag_started_us);
	}
...
static void process_rx_pkt(interface_buffer_handle_t *buf_handle)
{
	...
	uint16_t payload_len = 0;
	int ret = 0;
#if CONFIG_ESP_HOSTED_WIFI_TX_RETRY_ENABLED && !CONFIG_IDF_TARGET_ESP32C61
	...
	if (buf_handle->if_type == ESP_STA_IF && station_connected) {
...
#if CONFIG_ESP_HOSTED_WIFI_TX_RETRY_ENABLED && CONFIG_IDF_TARGET_ESP32C61
		/* Retain the SDIO frame while the bounded Wi-Fi TX pool is full.
		 * Returning its buffer after two attempts silently loses accepted host
		 * packets. Backpressure stays in this owner, without growing either
		 * pool. Only NO_MEM is recoverable; the finite deadline is below one
		 * 15 fps frame interval. The 20 ms trial still dropped queued frames. */
		const int64_t tx_start = esp_timer_get_time();
		bool waited = false;
		do {
			ret = esp_wifi_internal_tx(WIFI_IF_STA, payload, payload_len);
			if (ret != ESP_ERR_NO_MEM ||
			    esp_timer_get_time() - tx_start >= C61_WIFI_TX_BACKPRESSURE_US) {
				break;
			}
			waited = true;
			vTaskDelay(1);
		...
		if (waited) {
			c61_tx_probe_backpressure(ret, (uint32_t)(esp_timer_get_time() - tx_start));
		}
#elif CONFIG_ESP_HOSTED_WIFI_TX_RETRY_ENABLED
		do {
			ret = esp_wifi_internal_tx(WIFI_IF_STA, payload, payload_len);
			if (ret) {
...
void app_main(void)
{
	...
	ESP_ERROR_CHECK(c61_rx_probe_init());
